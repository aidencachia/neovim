...
local function update_mr_scroll()
    ...
    fn.system('git rev-parse --verify origin/main 2>/dev/null')
    local base_branch = vim.v.shell_error == 0 and 'origin/main' or 'origin/master'

    -- diff vs your MR base
    local cmd         = 'git diff ' .. base_branch .. ' -U0 -- ' .. fn.shellescape(file)
