...
local function lsp_attach(client, bufnr) -- runs on LspAttach :contentReference[oaicite:8]{index=8}
    ...
    -- inside your on_attach(client, bufnr) callback:
    if client.name == "ltex" then
        vim.keymap.set("n", "<leader>ldw", function()
            vim.lsp.buf.code_action({
                context = {
                    only = {
                        "source.addToDictionary",
                        "source.addToWorkspaceDictionary",
                    },
                    diagnostics = {}
                },
            })
        end, { buffer = bufnr, desc = "LTeX: add word to dictionary" })
    end
end
